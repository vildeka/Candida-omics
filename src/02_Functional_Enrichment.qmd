...
plot_weights_pathways <- function(
  ...
  ggplot(df, aes(x = .data[[x_axis]], y = pathway)) +
    ...
    {
      if (grepl("scaled_weight", x_axis)) xlim(c(0, 1.1))
    } +
...
```{r plot-indvidual-pathways}
# ============================================================
# Get scaled weights
# ============================================================
gene_df <- bind_rows(gene_enrich[1:2], .id = "sign") %>%
  as_tibble() %>%
  select(genes = "pathway", scaled_weight = feature.statistics_Factor1) %>%
  filter(scaled_weight != 0)
# ============================================================
# Get weights and gene groups (clus)
# ============================================================
...
weights <- get_weights(MOFAmodel, as.data.frame = TRUE) %>%
  ...
  mutate(clus = ifelse(factor == "Factor1", groups[.$genes], NA)) %>%
...
# ============================================================
phtw <- enrichment_list$neg$GO$sigPathways[[1]] %>%
...
# plot pos weights
# ============================================================
# phtw <- enrich.pos$GO$sigPathways[[1]] %>% split(., ceiling(seq_along(.) / 5))
phtw <- rownames(ht_list_pos$matrix) %>% split(., ceiling(seq_along(.) / 5))
...
plot_top_weights_tidy <- function(
  ...
  gr <- unique(W$clus)
  # Plot
  p <- ggplot(W, aes(x = genes_id, y = weight, color = clus)) +
    geom_point(size = 2) +
    geom_segment(
...
plot_lst <- phtw_genes %>%
  mutate(
    match = pmap(., ~ intersect(filt_wght$genes, ..2)),
    plots = pmap(
      .,
      ~ plot_genes_by_clus(filter(filt_wght, genes %in% ..2), order = T)
    ),
    plots = map2(pathway, plots, ~ .y + ggtitle(.x))
  ) %>%
  mutate(height = pmap_chr(., ~ (0.08 * length(..3)) + 2))
plot_lst

# quartz(width = 4, height = (0.1 * 10) + 2, dpi = 150) # 1.5
# plot_lst$plots[[9]]
# ============================================================
...
plot_genes_by_clus(
  filter(weights_df, genes %in% phtw_genes$genes[[2]]),
  order = TRUE
)
```