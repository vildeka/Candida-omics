...
plot_weights_pathways <- function(weights_df, pathways, n_label = 5) {
  ...
  df <- weights_df %>%
    filter(pathway %in% phtw) %>% # assumes you have pathway column
  ...
  ggplot(df, aes(x = value_scaled, y = pathway)) +
    ...
    geom_point(data = df %>% filter(!top), color = "grey70", size = 1.5, ) +
