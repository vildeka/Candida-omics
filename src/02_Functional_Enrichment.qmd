...
ht_list_neg <- make_gsea_heatmap(
  ...
  gene_treshold = 0,
  cell_width = 0.1,
  file = "../Results/GSEA_heatmap_neg_3.png",
  width = 10,
  k_clusters = 4
)
...
ht_list_pos <- make_gsea_heatmap(
  ...
  gene_treshold = 0,
  file = "../Results/GSEA_heatmap_both.png",
  width = 12,
  k_clusters = 6
)
```
...
plot_weights_pathways <- function(weights_df, phtw, n_label = 5) {
  wgt <- filter(weights, factor == "Factor1") %>%
    select(genes = "feature", weight = "value", clus, factor)

  weights_df <- enrich.neg$GO$feature.sets %>%
    as_tibble(rownames = "pathway") %>%
    pivot_longer(-pathway, names_to = "genes") %>%
    filter(value == 1) %>%
    mutate(Ngenes = n(), .by = "pathway") %>%
    left_join(., wgt, by = "genes") %>%
    left_join(., result$neg$GO, by = "pathway") %>%
    arrange(pval.adj_Factor1)

  df <- weights_df %>%
    filter(pathway %in% phtw) %>%
    mutate(value_scaled = weight / max(abs(weight)), .by = "factor") %>%
    mutate(rank = rank(-abs(value_scaled)), .by = "pathway") %>%
  ...
  df_labels <- df %>%
    group_by(pathway) %>%
    slice_max(order_by = abs(value_scaled), n = n_label) %>%
    ungroup()

  position = position_jitter(width = 0, height = 0.4, seed = 123)

  ggplot(df, aes(x = value_scaled, y = pathway)) +
    geom_violin(
      # bounds = c(-0.7, 0.7),
    ...
    xlim(min(df$value_scaled), max(df$value_scaled) + 0.1)
}
...
plt_list <- map(phtw, ~ plot_weights_pathways(weights, .x))

ggsave(
  filename = paste0("../Results/GO_enrichmentplots.pdf"),
...
plot_weights_pathways(weights, c("GOBP_DEFENSE_RESPONSE"))
