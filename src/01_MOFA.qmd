...
FM_path <- "../Tidy_data/Metagenomics/Functional/Functional_annotation_CLR_filtered.RDS"
...
# select pathway id colum
FM_matrix <- FM_matrix$community_lvl %>%
...
views <- list(
  ...
  Functional = as.matrix(FM_matrix)
)
```
...
run_mofa_with_seed <- function(seed) {
  ...
  MOFAmodel <- run_mofa(
    MOFAobject,
    outfile = paste0(
      "../Results/MOFA/MOFA_allBL_noS84_1e-6_seed_",
      seed,
      ".hdf5"
    ),
    use_basilisk = TRUE
...
write_csv(elbo_table, "../Results/MOFA/elbo_table_noS84_1e-6.csv")
...
elbo_table <- read_csv("../Results/MOFA/elbo_table_noS84_1e-6.csv")
...
MOFAmodel <- load_model(
  ...
  paste0("../Results/MOFA/MOFA_allBL_noS84_1e-6_seed_", best_seed, ".hdf5")
)
