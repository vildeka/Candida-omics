...
ovw <- plot_data_overview(MOFAmodel) +
  ...
  labs(tag = "Samples") +
  theme(
    plot.tag.position = c(0.65, 0.97),
    plot.tag = element_text(colour = "#626262"),
    plot.margin = unit(c(-0, -0.2, 0.1, 0.15), "cm"),
...
p1 <- p1 +
  coord_flip() +
  labs(tag = "Variance explained") +
  theme(
    plot.tag.position = c(0.5, 0.97),
    plot.tag = element_text(colour = "#626262"),
    text = element_text(size = 8),
...
p2 <- p2 +
  ...
  theme(
    ...
    plot.tag = element_text(colour = "#626262"),
    text = element_text(size = 8),
    legend.position = c(0.5, 1.2), # 👈 just above panel, tight
...
#annotate("text", x = 5.5, y = 6, label = "Factors", colour = "gray")

# quartz(width = 5.5, height = 2.1)
